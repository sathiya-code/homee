...
export const homeBestFour = async (page, config = {}) => await get(URL.BEST_FOUR + '?page=' + page, config);
...
export const getVendorByType = async (type, page, config = {}) => await get(URL.GET_VENDOR_BY_TYPE + '?page=' + page + '&type=' + type, config);
export const getServicesTitle = async (config = {}) => await get(URL.GET_SERVICE_TITLE, config);
export const getUserAddress = async (config = {}) => await get(URL.HOME_DEFAULT_ADDRESS, config);
...
export const timeBasedMenu = async (config = {}) => await get(URL.TIME_BASED_MENU, config);
export const getDeliveryTime = async (config = {}) => await get(URL.DELIVERY_AVAILABLE_TIME, config);
export const setOneToOneCooks = async (config = {}) => await get(URL.SET_ONE_TO_ONE_COOKS, config);
export const paymentMethods = async (config = {}) => await get(URL.GET_PAYMENT_METHODS, config);
export const getSupportDetails = async (config = {}) => await get(URL.GET_SUPPORT_DETAILS, config);

//////PnD Service APIs//////////
export const getPndItemsAndConfigs = async (config = {}) => await get(URL.GET_PND_CONFIG, config);
export const getPndAddresses = async (config = {}) => await get(URL.GET_PND_ADDRESS, config);
export const getPndAddressById = async (id, config = {}) => await get(URL.GET_PND_ADDRESS_BY_ID + id, config);
export const pndAddAddresses = async (data, config = {}) => await post(URL.ADD_PND_ADDRESS, data);
export const pndCalculatePrice = async (pickupAddress, dropAddress) => await get(URL.CALCULATE_PND_PRICE + pickupAddress + "/" + dropAddress);
export const pndPlaceOrder = async (data = {}, config = {}) => await post(URL.PND_PLACE_ORDER, data, config);
export const pndOrderStatus = async (data = {}, config = {}) => await post(URL.PND_ORDER_STATUS, data, config);
export const getpndOrderInfo = async (data = {}, config = {}) => await post(URL.GET_PND_ORDER_INFO, data, config);
...
export const sendWAotp = async (data = {}, config = {}) => await post(URL.SEND_WA_OTP, data, config);
...
export const cook_profile = async (id, config = {}) => await get(URL.COOK_PROFILE + id, config);
export const getMenuList = async (data = {}, config = {}) => await post(URL.GET_MENU_LIST, data, config);
export const getMenuByCook = async (id, page, search = null) => await get(URL.GET_MENU_BY_COOK + id + "?page=" + page + '&search=' + search);
...
export const wallet_full_amount2 = async (data = {}, config = {}) => await post(URL.WALLET_FULL_AMOUNT2, data, config);
...
export const getActivityStatus = async (data = {}, config = {}) => await post(URL.ADD_ACTIVITY_HISTORY, data, config);


/////instant-order Post///////
export const placeOrder = async (data = {}, config = {}) => await post(URL.ORDER_CREATE, data, config);
export const placePodOrder = async (data = {}, config = {}) => await post(URL.POD_ORDER_CREATE, data, config);
export const paymentStatus = async (data = {}, config = {}) => await post(URL.PAYMENT_STATUS, data, config);
export const orderStatusCheck = async (data = {}, config = {}) => await post(URL.STATUS_CHECK, data, config);


///Pre-Order////
export const getPreOrderBanner = async (config = {}) => await get(URL.PRE_ORDER_BANNER, config);
export const getPreOrderCooks = async (config = {}) => await post(URL.PRE_ORDER_COOKS, config);
export const getFoodListPreOrder = async (config = {}) => await post(URL.FOOD_LIST_PRE_ORDER, config);
export const getTimingList = async (config = {}) => await get(URL.GET_TIMING_LIST, config);
export const preOrderCartAdd = async (config = {}) => await post(URL.PRE_ORDER_CART_ADD, config);
export const preOrderCartMinus = async (id, config = {}) => await get(URL.PRE_ORDER_CART_MINUS + id, config);
export const preOrderCartAddMore = async (config = {}) => await post(URL.PRE_ORDER_CART_ADD_MORE, config);
export const preOrderAddTiming = async (config = {}) => await post(URL.PRE_ORDER_ADD_TIMING, config);
export const preOrderAddDate = async (config = {}) => await post(URL.PRE_ORDER_ADD_DATE, config);
export const getPreOrderCart = async (config = {}) => await post(URL.PRE_ORDER_GET_CART, config);
export const preOrderCartRemove = async (config = {}) => await post(URL.PRE_ORDER_CART_REMOVE, config);
export const getPreOrderDateTimeDetails = async (config = {}) => await post(URL.PRE_ORDER_DATE_TIME_DETAILS, config);
export const preOrderAddCheckout = async (config = {}) => await post(URL.PRE_ORDER_ADD_CHECKOUT, config);
export const preOrderUpdateCheckout = async (config = {}) => await post(URL.PRE_ORDER_UPDATE_CHECKOUT, config);
export const preOrderHistory = async (id, config = {}) => await get(URL.PRE_ORDER_HISTORY, config);
export const preOrderCancel = async (id, config = {}) => await get(URL.PRE_ORDER_CANCEL + id, config);
export const preOrderCancelConfirm = async (config = {}) => await post(URL.PRE_ORDER_CANCEL_CONFIRM, config);


export const getServicableArea = async (config = {}) => await get(URL.GET_SERVICABLE_AREA, config);
    ...
    try {
        if (err?.response?.data) {
            ...
            if (status == 400) {
                ...
                {
                    !UnAuthenticated && Alert.alert('UnAuthenticated Access', 'Session Closed Close your app and Reopen', [
                        {
                            text: 'Close',
                            onPress: async () => {
                                await removeUnAuth();
                ...
                Alert.alert('Status Failed', `server returns ${status}`, ` msg: ${data?.message}`);
